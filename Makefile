...
all: bola.exe circles.exe passaro_andante.exe teclado.exe bouncer.exe louco.exe frogger.exe pong.exe passaro_raivoso.exe tela.exe allegro_base.exe enduro-0.exe tenismp.exe multitenis.exe
...
tenismp.exe: tenismp.o
	gcc -o tenismp.exe tenismp.o $(PATH_ALLEGRO)$(LIB_ALLEGRO)

tenismp.o: tenismp.c
	gcc -I $(PATH_ALLEGRO)$(INCLUDE_ALLEGRO) -c tenismp.c	
